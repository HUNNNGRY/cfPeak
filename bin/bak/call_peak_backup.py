        ...
        if local_max > min_cov:
            ...
            while (start > left_bound) and (x[start - 1] >= decay*local_max):
                ...
                if local_max < x[start]: 
                    max_index_tmp = start
                    local_max = x[start] # update local_max if x[start] may hihger than localmax, necessary when localmax*decay<min_cov
            end = max_index
            while (end < (len(x) - 1)) and (x[end + 1] >= decay*local_max):
                ...
                if local_max < x[end]: 
                    max_index_tmp = end
                    local_max = x[end] # update local_max if x[end] may hihger than localmax, necessary when localmax*decay<min_cov
            # add current peak to results
            if (end - start) >= min_peak_length:
                #print((start, end, local_max))
                peaks.append([start, end, local_max, max_index_tmp])
