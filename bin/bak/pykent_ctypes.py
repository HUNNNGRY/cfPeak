...
class SharedLibrary(object):
    ...
    def create_alias(self, name, new_name):
        self.symbols[new_name] = self.symbols[name]
...
def init_library(libpath='jkweb'):
    ...
    lib.create_alias('bbiFileClose', 'bigWigFileClose')
...
class BbiFile(object):
    def __init__(self, filename, sig, typeName):
        filename = bytes(filename, encoding='utf-8')
        self.bbi = bbiFileOpen(filename, sig, typeName)
        if not self.bbi:
            raise IOError('cannot open the bigwig file: ' + filename)
        self.lm = lmInit(0)
    
    def close(self):
        bbiFileClose(byref(self.bbi))
        self.bbi = None
    
    def __del__(self):
        if hasattr(self, 'bbi') and (self.bbi is not None):
            self.close()
    
    def get_chrom_size(self, chrom):
        return bbiChromSize(self.bbi, to_bytes(chrom))
    
    def get_chrom_list(self):
        chrom_list = []
        chrom_info_head = bbiChromList(self.bbi)
        chrom_info = chrom_info_head
        while chrom_info:
            chrom_info = chrom_info.contents
            chrom_list.append((to_str(chrom_info.name), chrom_info.size))
...
class BigWigFile(BbiFile):
    def __init__(self, filename):
        self.bbi = bigWigFileOpen(to_bytes(filename))
        if not self.bbi:
            raise IOError('cannot open the bigwig file: ' + filename)
        self.lm = lmInit(0)
    ...
    def is_bigwig(filename):
        return bool(isBigWig(to_bytes(filename)))
    
    @staticmethod
    def check_sigs(filename):
        return bool(bigWigFileCheckSigs(to_bytes(filename)))

    def query(self, chrom, start=0, end=None, dtype='float', fillna=None):
        if end is None:
            return self.values_on_chrom(chrom, dtype=dtype, fillna=fillna)
        else:
            return self.interval_query(chrom, start=start, end=end, dtype=dtype, fillna=fillna)
    ...
    def interval_query(self, chrom, start, end, dtype='float', fillna=None):
        ...
        """
        interval = bigWigIntervalQuery(self.bbi, to_bytes(chrom), start, end, self.lm)
        if fillna is None:
            fillna = np.nan
    ...
    def values_on_chrom(self, chrom, dtype='float', fillna=None):
        ...
        values = None
        if bigWigValsOnChromFetchData(p_chrom_vals, to_bytes(chrom), self.bbi):
    ...
    def interval_query_blocked(self, chrom, start, end, block_starts, block_sizes):
        ...
        for i, block_start, block_size in zip(range(n_blocks), block_starts, block_sizes):
            block_start += start
            interval = bigWigIntervalQuery(self.bbi, to_bytes(chrom),
...
if __name__ == '__main__':
    ...
    for chrom, size in chrom_sizes:
        values = bwf.values_on_chrom(chrom, dtype='int', fillna=0)
        print('interval_query {}:0-{}: {}'.format(chrom, size, ','.join(values.astype('str'))))