...
def _call_peaks_localmax(x, min_peak_length=10, bin_width=10, min_cov=2, decay=0.5):
    ...
    '''
    tx_cov = np.mean(x) # better not trim
        ...
        if local_max > min_cov:
            ...
            while (start > left_bound) and (x[start - 1]-tx_cov >= decay*(local_max-tx_cov)): # while (start > left_bound) and (x[start - 1] >= decay*local_max)
                ...
                if local_max < x[start]: 
                    max_index_tmp = start
                    local_max = x[start] # update local_max if x[start] may hihger than localmax
            end = max_index
            while (end < (len(x) - 1)) and (x[end + 1]-tx_cov >= decay*(local_max-tx_cov)): # decay*local_max
                ...
                if local_max < x[end]: 
                    max_index_tmp = end
                    local_max = x[end] # update local_max if x[end] may hihger than localmax
