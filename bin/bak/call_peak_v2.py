...
#bpf: resample reads to get cov that meets FDR cutoff
def estimate_bg_local(signals):
    '''signals
    '''
    signals_mean = np.asarray([np.mean(s) for s in signals])
    bg = np.median(signals_mean)
...
def _call_peaks_localmax(x, min_peak_length=10, bin_width=10, min_cov=2, decay=0.5):
    ...
    x: array-like, (length,)
        Input signal values (single base ?)
    ...
    np.random.seed(1234)
    tx_cov = np.mean(bin_cov) # better not trim
    subset = min(5000,bin_cov.shape[0]) # ctrl complexity of gini calc: no more than 200 pos/values
    tx_cov = tx_cov * (1-np.nan_to_num(gini(np.random.choice(bin_cov, subset, replace=False)))) # adjust tx_cov by gini(count), 
    # tx_cov = max(tx_cov,0)
    min_cov = max(min_cov,tx_cov)
    ...
    while cand_bin_index < n_cand_bins:
        i = cand_bins[cand_bin_index]*half_bin_width # central position of bin, consider only one localmax peak each 10nt bin
        start = i
        end = i
        # max_index_tmp = i
        # find local max, not limit to this bin
        ...
        if local_max > min_cov:
            ...
            while (end < (len(x) - 1)) and (x[end + 1]-tx_cov >= decay*(local_max-tx_cov)): # decay*local_max
                end += 1
            ...
            if (end - start) >= min_peak_length:
                ...
                peaks.append([start, end, local_max, max_index_tmp]) # why not need deduplicate, adjacent peak not overlap: below left_bound/end updated, also next_cand_bin may jump to far away
...
def call_peaks_localmax(args):
    ...
    for chrom, size in chroms.items():
        ...
        for peak in peaks_chrom:
            ...
            bed.write('%s\t%d\t%d\tpeak_%d\t%d\t+\t%d\n'%(chrom, peak[0], peak[1], n_peaks, peak[2], peak[3]))
    bigwig.close()
    bed.close()
