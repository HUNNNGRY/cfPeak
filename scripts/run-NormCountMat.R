...
# {
#   setwd("/BioII/lulab_b/baopengfei/projects/WCHSU-FTC/")
#   ma <- "/BioII/lulab_b/baopengfei/projects/WCHSU-FTC/output/TCGA_small7/call_peak_all/count_matrix/cfpeakCNN_b5_d50_p1.txt"
#   sa <- "all"
#   gr <- "FALSE" # "FALSE"
#   fi <- FALSE
#   me <- "none"
...
colnames(y.cpm)[1] <- "gene_id" # "feature"

# output
message(paste0("write file to ",ou))
write.table(y.cpm, file = ou, sep = "\t",col.names = T,row.names = F,quote = F)
#data.table::fwrite(y.cpm, file = ou, sep = "\t",col.names = T,row.names = F,quote = F)
