...
def realigner(in_bam, out_dir, max_hits=100, max_tags=-1, read_tagger_method='median', 
		winsize=50, unstranded=False, retag=False, strandness="same",seed=1234):
	...
	obam0 = pysam.Samfile(os.path.join(out_dir, 'allrealigned.bam'), 'wb', template = mbam)
	...
	logger.info('found %i mreads @ %i locations' % ( len(mread_dict), len(location_to_reads) ) )
	...
	for read_qname in mread_dict:
		...
		# logger.info("subgraph %i: |e|=%i, |v|=%i"%(subg_counter, len(read_to_locations), len(subgraph)) )
		...
		for read in multi_reads_weights:
			for node in multi_reads_weights[read]:
				...
				# random shuffle by bpf
				alignment.set_tag('AS', score)
				obam0.write(alignment) # write all alignments

				#only select best align record
				if score<1 and random.random() > score:
					continue	
				#change AS to 100% prob. here
				alignment.set_tag('AS', 1.000)    
				#change flag to primary in bash (too many if condition, may very slow in py)
	...
	obam0_sorted_fn = os.path.join(out_dir, 'allrealigned.sorted.bam')
	pysam.sort('-o', obam0_sorted_fn, os.path.join(out_dir, 'allrealigned.bam'))
	pysam.index(obam0_sorted_fn)
	os.remove(os.path.join(out_dir, 'allrealigned.bam'))
...
def parsering(args):
	...
	try:
		...
		realigner(in_bam, out_dir, max_hits=max_hits, max_tags=max_tags, read_tagger_method=tag_method, 
			winsize=winsize, unstranded=unstranded, retag=retag, strandness=strandness, seed=seed)
...
if __name__=='__main__':
	import argparse
	parser = argparse.ArgumentParser('realigner')

	# input/output
	parser.add_argument("-i", "--input", dest="in_bam", type=str, required=True,
		help="Input bam file")
	parser.add_argument("-o", "--out-dir", dest="out_dir", type=str, required=True,
		help="Output folder")
	# processing
	parser.add_argument("--read-tagger-method", dest="tag_method", type=str, 
	...
	parser.add_argument("--max-multihits", dest="max_hits", type=int, default=100,
	...
	parser.add_argument("--retag", dest="retag", default=False, action='store_true',
		help="Retag the bam regardless when turned on; invalid when no previous files found")
	# realign
	parser.add_argument("--winsize", dest="winsize", type=int, default=50,
		help="Local window size for em computations; default: 50")
	parser.add_argument("--unstranded", dest="unstranded", default=False, action="store_true",
		help="Unstranded alignments if turned on")
	parser.add_argument("--strandness", dest="strandness", type=str, 
		default="same", choices=['same', 'opposite', 'none'],
		help="The expected read strandness with transcription direction: same, opposite, or none(i.e. unstranded); default: same")
	parser.add_argument("--seed", dest="seed", type=int, default=1234,
		help="Seed for random select one representative alignment from multi-align records by likilyhood socre; default: 1234")
