...
#domain intersect with given bed6
# last 2207 by bpf 
# b.p.f@qq.com
...
parser$add_argument('-i', '--inputFile', type='character', default='-',
                    help='input domain BED file')
parser$add_argument('-b', '--inputBed6', type='character', required=T,
                    help='input region bed6 file')
parser$add_argument('-r', '--regionLabel', type='character', default='reg',
                    help='input region label')
...
## get region bed6 (tx/gn)
ago2 <- data.table::fread(inputBed6,data.table = F,sep = '\t',check.names = F,stringsAsFactors = F)
ago2 <- ago2[,1:6]
