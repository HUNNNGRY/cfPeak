...
threshold() {
	...
	TP=$(echo "$predicted_truth" | cut -f1-6 | bedtools intersect -split -s -u -a - -b $standard | wc -l) # -f 0.1 -F 0.1: require both >10% overlap, when piranha perform best
	FP=$(echo "$predicted_truth" | cut -f1-6 | bedtools intersect -split -s -v -a - -b $standard | wc -l)
	TN=$(echo "$predicted_false" | cut -f1-6 | bedtools intersect -split -s -u -a - -b $standard_false | wc -l) # -f 0.1 -F 0.1: require both >10% overlap, when piranha perform best
	FN=$(echo "$predicted_false" | cut -f1-6 | bedtools intersect -split -s -v -a - -b $standard_false | wc -l)
